...
## 프롬프트 엔지니어링
- 이미 존재하는 LLM을 원하는 작업을 할 수 있도록 조정하는 것
 
### 1. 제로샷 프롬프트
- 어떠한 example도 주지 않고 오로지 지시(instruction) 만으로 문제를 풀게 하는 방식
 
### 2. 사고의 연쇄 (CoT)
- LLM이 시간을 들여 사고하도록 프롬프트 앞에 LLM이 답데 도달하는 과정을 설명하도록 지시(instruction)을 삽입하는 방식
- 전반적으로 작업의 성능을 높임
...
### 5. 퓨샷 프롬프트
- 단순 지시뿐만 아니라 질문&정답 예제를 제공하여 문제를 푸는 패턴을 학습하도록 하는 방식
